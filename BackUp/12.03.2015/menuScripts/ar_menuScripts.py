...
def ar_switchMenus(passObj):
	if mc.objExists (passObj + ".IKFKpos"):
		...
		if 'ArmIKFK' in getswitchCtrl or 'LegIKFK' in getswitchCtrl:
			mc.menuItem(l = 'Switch IK <> FK' , c = lambda event:ar_snapIKFK(getswitchCtrl) , rp = 'N' ,  bld = True ,  ecr = 1)
...
def ar_mirrorCtrls(switchCtrl):
	...
	if 'ArmIKFK' in switchCtrl:
		getClavicle = mc.listConnections((switchCtrl + '.Clavicle'))[0]
...
#Function for Snaping IK to FK and vice versa
def ar_snapIKFK(switchCtrl):
	mc.undoInfo( ock = True )
...
def ar_setDynParent(setVal , cntrl):
	...
	mc.setAttr((cntrl + '.Parent%d' % getPB) , setVal)
